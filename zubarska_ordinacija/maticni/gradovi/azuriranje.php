...
?>
		...
		<main>
			<form name="unos" action="" method="post">
				<table class="tabelaUnos" align="center"  >
					<caption>UNOS NOVOG GRADA</caption>
					<tr>
						<td>Grad: </td>
						<td><input type="text" required name="naziv" value=<?php echo (isset($_REQUEST['naziv']))?$_REQUEST['naziv']:"";?>></td>
					</tr>
					<tr>
						<td colspan="2"><input type="submit" name="snimi" value="Snimi novi grad u bazu"></td>
					</tr>
				</table>
	...
	<?php
		if(isset($_REQUEST['snimi'])){
			...
			$sqlQuery = "INSERT INTO grad VALUES (null, '$naziv')";
			$res = mysqli_query($conn, $sqlQuery);
			if(!$res)
				greska("Grad nije upisan u bazu! <br> opis: ".mysqli_error($conn));
			else
	...
	?>
		...
		<main>
			<form name="unos" action="" method="post">
				<table class="tabelaUnos" align="center" >
					<caption>IZMENA GRADA</caption>
					<tr>
						<td>Grad: </td>
						<td><input type="text" required name="naziv" value="<?=$row['naziv'];?>"></td>
					</tr>
					<tr>
						<td colspan="2"><input type="submit" name="snimi" value="Snimi izmenu grada u bazu"></td>
					</tr>
				</table>
		...
		if(isset($_REQUEST['snimi'])){
			...
			$sqlQuery = "UPDATE grad SET naziv='$naziv' WHERE id=".$_REQUEST['id'];
			$res = mysqli_query($conn, $sqlQuery);
			if(!$res)
