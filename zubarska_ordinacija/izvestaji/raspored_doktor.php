...
<a href="../pocetna.php" class="nazad"><img src="../img/back.jpg" width=30 height=30></a>
	...
	<main>
			...
			<caption align="center" > RASPORED ZAKAZANIH PREGLEDA ZA 7 DANA OD <?=date('d.m.Y.')?> </caption>
		<?php
			...
			while($row = mysqli_fetch_assoc($res)){
				...
				$danpregleda = strtotime(date('Y-m-d',strtotime($row['vreme_start'])));
				$nedeljudana = strtotime(date("Y-m-d", strtotime("+1 week")));
				$danas = strtotime(date('Y-m-d'));
				 
			if($danpregleda >= $danas && $danpregleda <= $nedeljudana){			//za proveru rasporeda za nedelju dana
